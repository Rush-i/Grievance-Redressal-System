  ...
  useEffect(() => {
    const fetchComplaints = async () => {
      try {
        ...
        // Get the department from local storage
        const apoDepartment = localStorage.getItem('apoDepartment');

        // Filter complaints based on the department
              ...
              <StyledPaper key={complaint._id} sx={{ mb: 2 }}>
                ...
                {expandedComplaints[complaint._id] && (
                  <DetailBox>
                    ...
                    <Typography><strong>Details:</strong> {complaint.description || 'No additional details available'}</Typography>
...
export default GrievanceByDepartment;
